...
void measure_distance(){
  ...
  Serial.print("duraion r: ");
  Serial.println(duration_r);
  distance_in_cm_r = (duration_r/2.0) / 29.1;

  display_clear_show(String(distance_in_cm_l)+" "+String(distance_in_cm_r));
  Serial.println(String(distance_in_cm_l)+" "+String(distance_in_cm_r));
}

void measure_light_intensity(){
  int_left=(analogRead(A0)-int_adc0_c)/int_adc0_m;
  int_right=(analogRead(A2)-int_adc1_c)/int_adc1_m;
...
int done = 0;
void loop(){
   display_clear_show("parking");
   measure_distance();
   while ((distance_in_cm_l - distance_in_cm_r > 1) || (distance_in_cm_l - distance_in_cm_r < -1)){
   ...
   measure_distance();
   while ((distance_in_cm_l + distance_in_cm_r)/2 > 33){
    move_forward(100);
    measure_distance();
    display_clear_show("Moving");
   }

   
    display_clear_show("Done");
    delay(2000);
   ...
   while ((int_left - int_right) > 200 || (int_left - int_right) < -200){
    display_clear_show("there shall be light");
    if ((int_left - int_right) > 200){
      LEFT_2();
      delay(500);
    }else{
      RIGHT_2();
   ...
   measure_distance();
   while ((distance_in_cm_l + distance_in_cm_r)/2 > 12){
    move_forward(100);
    measure_distance();
    display_clear_show("Moving");
   }
